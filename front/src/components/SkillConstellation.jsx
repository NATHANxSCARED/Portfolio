...
const constellationSkills = [
  {
    ...
    description: "Des interfaces modernes, fluides et construites avec des composants réutilisables.",
  },
  {
    name: "Node.js",
    icon: SiNodedotjs,
    ...
    description: "Le moteur back-end de mes API, services et applications JavaScript.",
  },
  {
    name: "n8n",
    icon: SiN8N,
    ...
    description: "Des automatisations qui relient les outils et font circuler les données intelligemment.",
  },
  {
    name: "UI / UX",
    icon: SiFigma,
    ...
    description: "Des parcours simples et des interfaces soignées, pensés avant d’être développés.",
  },
  {
    name: "3D",
    icon: SiThreedotjs,
    ...
    description: "Des expériences web immersives avec Three.js, WebGL et une vraie profondeur visuelle.",
  },
  {
    name: "Sécurité",
    icon: FaShieldHalved,
    ...
    description: "Authentification, protection des données et bonnes pratiques intégrées dès la conception.",
  },
  {
    name: "WordPress",
    icon: SiWordpress,
    ...
    description: "Des sites administrables et personnalisés, adaptés aux besoins réels du client.",
  },
  {
    name: "Full-stack",
    icon: FaCode,
    ...
    description: "Une vision globale du produit, de la base de données jusqu’à l’expérience finale.",
  },
];
...
function SkillConstellation() {
  ...
  useEffect(() => {
    ...
    const coreWire = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: 0x74e8ff, wireframe: true, transparent: true, opacity: 0.24 })
    );
    ...
    constellationSkills.forEach((skill, index) => {
      ...
      const ring = new THREE.Mesh(
        ringGeometry,
        new THREE.MeshBasicMaterial({ color: skill.color, transparent: true, opacity: 0.28 })
      );
    ...
    constellationSkills.forEach((skill) => connectionPositions.push(0, 0, 0, ...skill.position));
    ...
    const connections = new THREE.LineSegments(
      connectionsGeometry,
      new THREE.LineBasicMaterial({ color: 0x3bc9ff, transparent: true, opacity: 0.16 })
    );
    ...
    for (let i = 0; i < dustCount; i += 1) {
      ...
      dustPositions[i * 3 + 2] = Math.sin(angle) * 2.5 - 1.5;
    }
    const dustGeometry = new THREE.BufferGeometry();
    dustGeometry.setAttribute("position", new THREE.BufferAttribute(dustPositions, 3));
    const dust = new THREE.Points(
      dustGeometry,
      new THREE.PointsMaterial({ color: 0xb9eaff, size: 0.025, transparent: true, opacity: 0.55 })
    );
    universe.add(dust);
    ...
    const handlePointerMove = (event) => {
      updatePointer(event);
      if (isDragging) {
        universe.rotation.y += (event.clientX - previousX) * 0.006;
        universe.rotation.x += (event.clientY - previousY) * 0.003;
        universe.rotation.x = THREE.MathUtils.clamp(universe.rotation.x, -0.35, 0.35);
    ...
    sceneStateRef.current = { camera, cameraTarget };

    const introAnimations = [];
    if (!reducedMotion) {
      universe.scale.setScalar(0.02);
      introAnimations.push(animate(universe.scale, {
        x: 1,
        y: 1,
        z: 1,
        duration: 1400,
      ...
      introAnimations.push(animate(shell.querySelectorAll(".constellation-control"), {
        opacity: [0, 1],
        y: [18, 0],
        delay: stagger(55),
        duration: 700,
  ...
  return (
    <section className="constellation" aria-labelledby="constellation-title">
      <div className="constellation-copy">
        <span className="constellation-eyebrow">Expérience interactive</span>
        <h3 id="constellation-title">Explorez ma constellation</h3>
        <p>Survolez une planète, cliquez pour l’explorer ou glissez dans l’espace.</p>
      </div>

      <div className="constellation-shell" ref={shellRef}>
        <canvas
          ref={canvasRef}
        ...
        />
        <div
          className={`constellation-core-label ${selectedSkill ? "is-hidden" : ""}`}
          aria-hidden="true"
        >
          <span>NATHAN</span>
        ...
        {selectedSkill && (
          <aside
            ...
            style={{ "--skill-color": selectedSkill.cssColor }}
            aria-live="polite"
          >
            <button
              type="button"
        ...
        <div className="constellation-controls" aria-label="Choisir une compétence">
          {constellationSkills.map((skill, index) => {
            const Icon = skill.icon;
            return (
              <button
                type="button"
                className={`constellation-control ${selectedIndex === index ? "is-active" : ""}`}
                key={skill.name}
                style={{ "--skill-color": skill.cssColor }}
